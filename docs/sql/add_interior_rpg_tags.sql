-- Tags cho tileset / interior RPG (top-down, life sim, nội thất hiện đại).
-- Chạy: dotnet run --project BE/scripts/SeedInteriorRpgTags
...
BEGIN
  ...
  IF style_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Cozy', 'cozy')) AS t(name, slug)
    LOOP
  ...
  IF genre_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Life Sim', 'life-sim')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
  ...
  IF technical_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Top-Down', 'top-down')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
