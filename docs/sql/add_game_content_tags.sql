-- Thêm tags: tile (Kỹ thuật & asset), character (Chủ đề).
-- platformer và oopi đã có sẵn trong DB.
-- Chạy: dotnet run --project BE/scripts/SeedGameContentTags
...
BEGIN
  ...
  IF technical_group_id IS NOT NULL THEN
    FOR tag_rec IN SELECT * FROM (VALUES ('Tile', 'tile')) AS t(name, slug)
    LOOP
  ...
  IF theme_group_id IS NOT NULL THEN
    FOR tag_rec IN SELECT * FROM (VALUES ('Character', 'character')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
