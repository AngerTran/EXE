-- Tags cho texture / tileset pixel (seamless, pattern, 8-bit…).
-- Chạy: dotnet run --project BE/scripts/SeedTextureTilesetTags
...
BEGIN
  ...
  IF style_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('8-Bit', '8-bit')) AS t(name, slug)
    LOOP
