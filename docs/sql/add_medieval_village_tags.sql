-- Tags cho kit môi trường 3D medieval / modular (Quaternius Medieval Village MegaKit…).
-- Chạy: dotnet run --project BE/scripts/SeedMedievalVillageTags
...
BEGIN
  ...
  IF style_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Low Poly', 'low-poly')) AS t(name, slug)
    LOOP
  ...
  IF genre_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('RPG', 'rpg')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
  ...
  IF technical_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Modular', 'modular')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
