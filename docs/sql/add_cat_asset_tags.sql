-- Tags cho asset 2D pet / mèo (Cat Asset Pack và tương tự).
-- Chạy: dotnet run --project BE/scripts/SeedCatAssetTags
...
BEGIN
  ...
  IF style_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Cute', 'cute')) AS t(name, slug)
    LOOP
  ...
  IF technical_group_id IS NOT NULL THEN
    FOR tag_rec IN
      SELECT * FROM (VALUES ('Props', 'props')) AS t(name, slug)
    LOOP
      IF EXISTS (SELECT 1 FROM public.tags WHERE slug = tag_rec.slug) THEN
