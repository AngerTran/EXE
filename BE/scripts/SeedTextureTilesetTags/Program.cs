...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_texture_tileset_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_texture_tileset_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_texture_tileset_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('8-bit', 'tileset', 'sprite-sheet', 'seamless', 'pattern', 'textures', 'tile')
...
Console.WriteLine("Texture / tileset tags:");
