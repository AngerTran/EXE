...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_audio_music_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_audio_music_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_audio_music_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('music-pack', 'bgm', 'soundtrack', 'chiptune')
...
Console.WriteLine("Audio music tags:");
