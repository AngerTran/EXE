...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_cat_asset_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_cat_asset_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_cat_asset_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('cute', 'pet', 'cat', 'animal', 'props')
...
Console.WriteLine("Cat / pet asset tags:");
