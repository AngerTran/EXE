...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_multi_platformer_tileset_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_multi_platformer_tileset_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_multi_platformer_tileset_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN (
      'pico-8', 'grassland', 'desert', 'snow', 'cave', 'factory', 'stronghold', 'biome',
      'parallax', 'side-scroller', 'spritesheet',
      'platformer', 'tileset', '16x16', 'auto-tile', 'animated', 'modular', 'pixel-art'
    )
...
Console.WriteLine("Multi Platformer tileset tags:");
