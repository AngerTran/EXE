...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_furniture_bits_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_furniture_bits_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_furniture_bits_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('office', 'home', 'furniture', 'interior', 'life-sim', 'cozy', 'low-poly', 'props', 'environment')
...
Console.WriteLine("Furniture bits tags:");
