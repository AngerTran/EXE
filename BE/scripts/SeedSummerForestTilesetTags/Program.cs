...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_summer_forest_tileset_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_summer_forest_tileset_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_summer_forest_tileset_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN (
      'forest', 'summer', 'waterfall', 'mushroom', 'wilderness',
      'snes', 'adventure', '16x16', 'auto-tile', 'animated',
      'tileset', 'top-down', 'tiled', 'pixel-art', 'rpg', 'nature'
    )
...
Console.WriteLine("Summer forest tileset tags:");
