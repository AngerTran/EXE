...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_dungeon_gathering_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_dungeon_gathering_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_dungeon_gathering_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN (
      'dungeon', 'castle', 'knight', 'monster', 'door', 'potion',
      'icons', 'playable-character', '4-direction', 'breakable', 'icon',
      'props', 'character', 'tileset', '16x16', 'animated', 'top-down',
      'hud', 'pixel-art', 'rpg', 'horror', 'medieval', 'interior'
    )
...
Console.WriteLine("Dungeon Gathering tags:");
