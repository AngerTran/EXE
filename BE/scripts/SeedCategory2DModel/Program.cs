...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_category_2d_model.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_category_2d_model.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_category_2d_model.sql");
}
...
{
    Console.WriteLine($"  [{reader.GetInt16(2)}] {reader.GetString(0)} — {reader.GetString(1)}");
}
