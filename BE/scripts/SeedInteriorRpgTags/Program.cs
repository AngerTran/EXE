...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_interior_rpg_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_interior_rpg_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_interior_rpg_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('cozy', 'life-sim', 'interior', 'furniture', 'environment', 'top-down')
...
Console.WriteLine("Interior RPG tags:");
