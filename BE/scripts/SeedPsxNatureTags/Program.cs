...
{
    var fromBe = Path.GetFullPath(Path.Combine(beRoot, "..", "docs", "sql", "add_psx_nature_tags.sql"));
    if (File.Exists(fromBe))
        return fromBe;

    var fromRepo = Path.GetFullPath(Path.Combine(beRoot, "..", "..", "docs", "sql", "add_psx_nature_tags.sql"));
    if (File.Exists(fromRepo))
        return fromRepo;

    throw new FileNotFoundException("Could not find docs/sql/add_psx_nature_tags.sql");
}
...
const string verifySql = """
    ...
    WHERE t.slug IN ('psx', 'tree', 'vegetation', 'nature', 'environment', 'retro', 'low-poly')
...
Console.WriteLine("PSX / nature tags:");
